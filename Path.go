...
func (p *Path) ReadFile(fileName, pathName string) (error) {
	...
	regData  = regexp.MustCompile(`^ *([+-]?[0-9]+\.[0-9]+) +([+-]?[0-9]+\.[0-9]+) +([+-]?[0-9]+\.[0-9]+) +([0-9]+) *$`)
	...
	for scanner.Scan() {
		...
		if inBlock {
			if regData.MatchString(line) {
				...
				x, _  = strconv.ParseFloat(matches[1], 10)
				y, _  = strconv.ParseFloat(matches[2], 10)
				w, _  = strconv.ParseFloat(matches[3], 10)
...
//
func (p *Path) AddView(x, y, w float64, it int) {
	var v *View
